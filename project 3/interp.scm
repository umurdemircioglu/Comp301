    ...
    (lambda (exp env)
      (cases expression exp
        ...
        (stack-push-exp (exp1 exp2)
                        (let ((stk (expval->arr (value-of exp1 env)))
                              (val (value-of exp2 env)))
                          (stack-push stk val)))

        (stack-size-exp (exp1)
           (let ((stk (expval->arr (value-of exp1 env))))
             (num-val (stack-size stk))))
       ...
       ;;;;;;;;;;;;;;;;;;;;;;;;; queue ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
        ...
        (queue-push-exp (exp1 exp2)
                        (let ((q (expval->arr (value-of exp1 env)))
                              (val (value-of exp2 env)))
                          (queue-push q val)))

        (queue-size-exp (exp1)
           (let ((q (expval->arr (value-of exp1 env))))
             (num-val (queue-size q))))
