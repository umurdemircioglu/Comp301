...
;
;
      (stack-test1 "let x = newstack() in begin stack-push(x, 10); stack-push(x, 20); stack-pop(x); stack-size(x) end" 1)
      ...
      (stack-test6 "let x = newstack() in begin stack-push(x, 10); print-stack(x) end" 0)
;
;            ; ==================== Queue test cases =========================;
;
      ...
      (queue-test6 "let x = newqueue() in begin queue-push(x, 10); print-queue(x) end" 0)
      ))
  )
