  ...
  (define value-of/k
    (lambda (exp env cont)
      (cases expression exp
        ...
        (null?-exp (exp1)
          (value-of/k exp1 env
            (null?-cont exp1 env cont)))
        ; Implement emptylist expression case here
        (emptylist-exp ()
           (value-of/k (emptylist-val '()) env cont))
  ...
  (define apply-cont
    (lambda (cont val)
      (cases continuation cont
        ...
        (car-cont (val1 saved-cont)
          (let ((v (expval->car val1)))
            (apply-cont v saved-cont)))
        ; implement "cdr-cont" continuation here
         (cdr-cont (val1 saved-cont)
          (let ((v (expval->cdr val1)))
            (apply-cont v saved-cont)))
        ; implement "null?-cont" continuation here
         (null?-cont (exp1 saved-env saved-cont)
          (let ((v (expval->null? exp1)))
           (apply-cont v saved-cont)))
                     ;#t)
        ...
        (list-cont (exp1 exps saved-env  saved-cont)
          (value-of/k exp1 saved-env
            (list-cont (cadr exps)) (cdr exps) saved-env saved-cont))
